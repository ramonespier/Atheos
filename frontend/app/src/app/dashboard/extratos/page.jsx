...
import { 
  FiEdit2, FiTrash2, FiX, FiPlus, FiLoader, FiAlertTriangle, FiChevronDown, 
  FiSearch, FiTrendingUp, FiTrendingDown, FiArchive, FiTarget, FiCalendar 
} from "react-icons/fi";
...
function TransactionItem({ transacao, isSubmitting, onEdit, onDelete }) {
  ...
  return (
    <motion.li layout variants={itemVariants} exit={{ opacity: 0, x: -30 }} className="group relative bg-slate-900 rounded-xl border border-slate-800/80 hover:border-orange-500/30 transition-colors duration-300">
      <div className="flex items-center justify-between p-4">
        <div className="flex items-center gap-4">
          <div className={clsx("w-10 h-10 rounded-lg flex items-center justify-center shrink-0", transacao.tipo === 'entrada' ? "bg-green-500/10 text-green-400" : "bg-red-500/10 text-red-400")}>
        ...
        <div className="flex items-center gap-2 sm:gap-6">
          ...
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button disabled={isSubmitting || isMetaTx} onClick={() => onEdit(transacao)} className="p-2 text-slate-400 hover:text-orange-400 disabled:text-slate-600 disabled:cursor-not-allowed" title={isMetaTx ? "Transações de metas não podem ser editadas" : "Editar Transação"}>
              <FiEdit2 />
            </button>
            <button disabled={isSubmitting || isMetaTx} onClick={() => onDelete(transacao)} className="p-2 text-slate-400 hover:text-red-400 disabled:text-slate-600 disabled:cursor-not-allowed" title={isMetaTx ? "Transações de metas não podem ser excluídas" : "Excluir Transação"}>
              <FiTrash2 />
            </button>
          </div>
        </div>
...
export default function Extratos() {
  ...
  const buscarTransferencias = async () => {
    // Não precisa mais do setIsLoadingData(true) aqui, para evitar piscar a tela
  ...
  useEffect(() => {
    ...
    const buscarUsuarioETransacoes = async () => {
      setIsLoadingData(true);
      try {
        const resUser = await fetch('http://localhost:3001/usuario/autenticado', { headers: { 'Authorization': `Bearer ${token}` } });
    ...
    buscarUsuarioETransacoes();

    const handleTransacoesUpdate = () => {
      toast.success("Novas movimentações detectadas!", {
        icon: '🔄',
    ...
    window.addEventListener('transacoesAtualizadas', handleTransacoesUpdate);

    return () => {
      window.removeEventListener('transacoesAtualizadas', handleTransacoesUpdate);
    };
  }, [router]);
  ...
  const handleFormSubmit = async (evento, isEdit = false) => {
    ...
    try {
      ...
      if (isEdit) setEditTransacao(null);
  ...
  const excluirTransacao = async () => {
    ...
    finally { setIsSubmitting(false); setTransactionToDelete(null); }
  };

  const transacoesAgrupadas = useMemo(() => {
    const isMetaTx = (t) => t.nome?.startsWith('Depósito na Meta:') || t.nome?.startsWith('Retirada da Meta:');

    const transacoesFiltradas = transacoes
      .filter(t => filter === 'all' || t.tipo === filter)
      .filter(t => t.nome.toLowerCase().includes(searchTerm.toLowerCase()));

    const txMetas = transacoesFiltradas.filter(isMetaTx);
    const txNormais = transacoesFiltradas.filter(t => !isMetaTx(t));
    ...
    const gruposNormais = txNormais.reduce((acc, t) => {
      ...
      else if (data.toDateString() === ontem.toDateString()) grupo = 'Ontem';
      else {
    ...
    return { normais: gruposNormais, metas: txMetas };
  ...
  return (
    <div className="flex font-sans bg-slate-950 text-slate-100 min-h-screen">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 lg:gap-8">
              <motion.div variants={containerVariants} initial="hidden" animate="visible">
                <div className="flex items-center gap-3 text-2xl font-bold text-slate-300 mb-6 border-b-2 border-slate-700 pb-3">
                  <FiCalendar /><h2>Transações Gerais</h2>
                </div>
                {isLoadingData ? (<div className="text-center py-16"><FiLoader className="animate-spin text-orange-500 mx-auto" size={48}/></div>
                  ...
                  Object.entries(transacoesAgrupadas.normais).map(([grupo, items]) => (
                    <section key={grupo} className="mb-8">
                      ...
                      <ul className="space-y-3"><AnimatePresence>{items.map(transacao => <TransactionItem key={transacao.id} transacao={transacao} isSubmitting={isSubmitting} onEdit={setEditTransacao} onDelete={setTransactionToDelete} />)}</AnimatePresence></ul>
                    </section>
                  ))
              ...
              <motion.div variants={containerVariants} initial="hidden" animate="visible">
                <div className="flex items-center gap-3 text-2xl font-bold text-slate-300 mb-6 border-b-2 border-slate-700 pb-3">
                  <FiTarget /><h2>Movimentações de Metas</h2>
                </div>
                {isLoadingData ? (<div className="text-center py-16"><FiLoader className="animate-spin text-orange-500 mx-auto" size={48}/></div>
                ) : transacoesAgrupadas.metas.length > 0 ? (
                  <section><ul className="space-y-3"><AnimatePresence>{transacoesAgrupadas.metas.map(transacao => <TransactionItem key={transacao.id} transacao={transacao} isSubmitting={isSubmitting} onEdit={() => {}} onDelete={() => {}} />)}</AnimatePresence></ul></section>
      ...
      <AnimatePresence>
        {editTransacao && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[50] p-4" onClick={() => setEditTransacao(null)}>
            <motion.div variants={modalVariants} initial="hidden" animate="visible" exit="exit" onClick={(e) => e.stopPropagation()} className="w-full max-w-md bg-slate-900 rounded-2xl border border-slate-700 p-6">
              <form onSubmit={(e) => handleFormSubmit(e, true)}>
                <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold text-orange-400">Editar Transação</h3><button type="button" onClick={() => setEditTransacao(null)} className="p-1 text-slate-400 hover:text-white"><FiX /></button></div>
      ...
      <AnimatePresence>
        {transactionToDelete && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[50] p-4" onClick={() => setTransactionToDelete(null)}>
            <motion.div variants={modalVariants} initial="hidden" animate="visible" exit="exit" onClick={(e) => e.stopPropagation()} className="w-full max-w-md bg-slate-900 rounded-2xl border border-slate-700 p-6 text-center">
              ...
              <p className="text-slate-400 mb-6">Tem certeza que deseja excluir a transação "{transactionToDelete.nome}"? <br/> A ação é irreversível.</p>
              <div className="flex justify-center gap-4">
                <button type="button" onClick={() => setTransactionToDelete(null)} className="font-semibold py-3 px-6 rounded-xl bg-slate-700 hover:bg-slate-600 text-slate-200 w-full">Cancelar</button>
                <motion.button type="button" onClick={excluirTransacao} disabled={isSubmitting} className="font-semibold py-3 px-6 rounded-xl bg-gradient-to-r from-red-500 to-red-600 text-white shadow-lg disabled:opacity-50 w-full" whileTap={{ scale: 0.95 }}>{isSubmitting ? <FiLoader className="animate-spin mx-auto" /> : 'Excluir'}</motion.button>
              </div>
            </motion.div>
          </motion.div>
        )}
